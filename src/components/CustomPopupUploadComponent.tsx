...
import { Box, Button, Input, Label, Text, CheckBox } from '@adminjs/design-system';
  ...
  return (
    <Box backgroundColor="white" p={32} borderRadius={4} boxShadow="card">
      <form style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }} onSubmit={handleUpload}>
        ...
        <Box width={1}>
          <Label htmlFor="linkUrl" style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            Link Url
          </Label>
          <Input width={1} variant="default" type="text" id="linkUrl" value={linkUrl} onChange={handleLinkUrlChange} />
        </Box>
        <Box width={1} display="flex" alignItems="center" gap={2}>
          <CheckBox id="isActive" checked={isActive} onClick={() => setIsActive(!isActive)} />
